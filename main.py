...
def delete_folder(
    ...
    if items:
        supabase.storage.from_(BUCKET).remove(
            [f"{prefix}/{i['name']}" for i in items]
        )
...
async def upload_file(
    ...
    db_folder_id = None if folder_id in (None, "root") else int(folder_id)
    storage_folder = folder_id or "root"

    storage_path = f"{user_id}/{storage_folder}/{file.filename}"
...
def delete_file(
    ...
    db_folder_id = None if folder_id == "root" else int(folder_id)
    storage_path = f"{user['user_id']}/{folder_id}/{filename}"

    supabase.storage.from_(BUCKET).remove([storage_path])
    crud.delete_file(session, user["user_id"], db_folder_id, filename)

    return {"message": "Deleted"}


# =========================================================
# FILE RENAME (CRITICAL FIXED)
# =========================================================
...
def rename_file(
    ...
    old_path = f"{user_id}/{folder_id}/{filename}"
    new_path = f"{user_id}/{folder_id}/{new_name}"
    ...
    file = session.exec(
        select(FileModel).where(
            ...
            FileModel.folder_id == db_folder_id,
            FileModel.name == filename
        )
    ).first()

    if file:
        meta = json.loads(file.meta)
        meta["originalName"] = new_name.replace(".enc", "")
        file.name = new_name
        file.meta = json.dumps(meta)
        session.add(file)
        session.commit()
